...
public class NetCacheUtils {
    ...
    public NetCacheUtils(Handler handler, LocalCacheUtils lCacheUtils, MemoryCacheUtils mMemoryUtils) {
     ...
     */
    public void requestBitmap(String imageUrl, int position) {
//		new Thread(new InternalRunnable(imageUrl, position)).start();
    ...
    private class InternalRunnable implements Runnable {
        ...
        InternalRunnable(String imageUrl, int position) {
        ...
        public void run() {
            ...
            try {
                ...
                if (responseCode == 200) {
                    ...
                    // 向内存存一份
                    mMemoryUtils.putBitmap(imageUrl, bm);
                    // 向本地存一份
                    lCacheUtils.putBitmap(imageUrl, bm);
                }
            } catch (Exception e) {
                e.printStackTrace();
                Message msg = mHandler.obtainMessage();
